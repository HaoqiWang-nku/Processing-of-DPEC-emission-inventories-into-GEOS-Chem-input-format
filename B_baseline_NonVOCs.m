...
for y = 1:length(years);
    for m = 1:length(months);
        ...
        month = months{m};
       % 定义物种列表
       species_list_for_SAPRC = {'NOx', 'CO', 'SO2', 'NH3', 'BC', 'OC', 'PM25'  };
       species_list_for_GC    = {'NOx', 'CO', 'SO2', 'NH3', 'BC', 'OC', 'PMFINE'  }; 

       % 基础路径
       base_path = 'Z:\DEPC\v1.2\V1.2SAPRC99\on-time_peak-net_zero-clean_air_SAPRC99\on-time_peak-net_zero-clean_air\grid\on-time_peak-net_zero-clean_air';
       ...
       for i = 1:length(species_list_for_SAPRC)
           current_species = species_list_for_SAPRC{i};
             path{i}.agriculture= fullfile(base_path, year, [year '_' month '_agriculture_' current_species '.nc']);
             path{i}.industry= fullfile(base_path, year, [year '_' month '_industry_' current_species '.nc']);
             path{i}.power= fullfile(base_path, year, [year '_' month '_power_' current_species '.nc']);
             path{i}.residential= fullfile(base_path, year, [year '_' month '_residential_' current_species '.nc']);
             path{i}.transportation= fullfile(base_path, year, [year '_' month '_transportation_' current_species '.nc']);
       end
       % 输出路径
       ...
       for i = 1:length(species_list_for_SAPRC)
           agr_f{i} = (agr_o_reshaped{i}).*1000./China_map./31./24./3600;
           ind_f{i} = (ind_o_reshaped{i}).*1000./China_map./31./24./3600;
           pow_f{i} = (pow_o_reshaped{i}).*1000./China_map./31./24./3600;
           res_f{i} = (res_o_reshaped{i}).*1000./China_map./31./24./3600;
           tra_f{i} = (tra_o_reshaped{i}).*1000./China_map./31./24./3600;
       end
       ...
       %NETCDF文件读写
       % 定义文件名
       outputFile = fullfile(['./DDDD_on-time_peak-net_zero-clean_air/MEIC_0.25_NonVOC_' year '_' month '.nc']);
       ...
       for i = 1:length(species_list_for_GC)
           ...
           var_name_tra = [current_species '_transportation'];
            ...
            % 定义当前变量
            varid_current_agr = netcdf.defVar(ncid, var_name_agr, 'float', [dimid_x, dimid_y]);
            varid_current_ind = netcdf.defVar(ncid, var_name_ind, 'float', [dimid_x, dimid_y]);
            varid_current_pow = netcdf.defVar(ncid, var_name_pow, 'float', [dimid_x, dimid_y]);
            varid_current_res = netcdf.defVar(ncid, var_name_res, 'float', [dimid_x, dimid_y]);
            varid_current_tra = netcdf.defVar(ncid, var_name_tra, 'float', [dimid_x, dimid_y]);
        
            % 添加单位属性为 "kgC/m2/s"
            netcdf.putAtt(ncid, varid_current_agr, 'units', 'kg/m2/s');
            netcdf.putAtt(ncid, varid_current_ind, 'units', 'kg/m2/s');
            netcdf.putAtt(ncid, varid_current_pow, 'units', 'kg/m2/s');
            netcdf.putAtt(ncid, varid_current_res, 'units', 'kg/m2/s');
            netcdf.putAtt(ncid, varid_current_tra, 'units', 'kg/m2/s');
        end

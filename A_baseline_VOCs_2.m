...
for y = 1:length(years);
    for m = 1:length(months);
        ...
        month = months{m};
       % 定义物种列表
       species_list_for_SAPRC = { 'BENZ','ACYE' };
       species_list_for_GC = { 'BENZ',  'C2H2'  }; 
       numC =  {'2', '6'};

       % 基础路径
       base_path = 'Z:\DEPC\v1.2\v1.2SAPRC07\on-time_peak-net_zero-clean_air-SAPRC07';
       ...
       for i = 1:length(species_list_for_SAPRC)
           current_species = species_list_for_SAPRC{i};
             path{i}.agriculture= fullfile(base_path, year, [year '_' month '_agriculture_SAPRC07_' current_species '.nc']);
             path{i}.industry= fullfile(base_path, year, [year '_' month '_industry_SAPRC07_' current_species '.nc']);
             path{i}.power= fullfile(base_path, year, [year '_' month '_power_SAPRC07_' current_species '.nc']);
             path{i}.residential= fullfile(base_path, year, [year '_' month '_residential_SAPRC07_' current_species '.nc']);
             path{i}.transportation= fullfile(base_path, year, [year '_' month '_transportation_SAPRC07_' current_species '.nc']);
       end
       % 输出路径
